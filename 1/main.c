/******************************************声明******************************************************/
////////////////////////////程序由bilibili川乐水兴（UID181818866）制作/////////////////////////////////
//////////////////我觉得这个程度大概也用不做商业用途，不过还是禁止的，仅供参考学习////////////////////////
///////////////////////////欢迎大家来B站支持我，给我最新的视频三连什么的////////////////////////////////
///////////////////////////////////我的GitHub号是yalexing/////////////////////////////////////////////
/////////////此程序最初是适用于proteus软件仿真的，可能直接移到实物上并不适用，这就需要你们改了////////////
/***************************************************************************************************/
#include <lcd12864.c>
#include "Delayms.h"
#include "Timer0.h"
...
void main(void)
{
	unsigned long int address =  0x000000;
	unsigned char xdata a[176];
	unsigned char xdata b[176];
	unsigned char xdata c[176];
	unsigned char xdata d[176];
	uint n = 64;
	...
	while(1)                                //动画开始
	{	
		SdReadBlock(a,address,176);
		address+=176;
		SdReadBlock(b,address,176);
		address+=176;
		SdReadBlock(c,address,176);
		address+=176;
		SdReadBlock(d,address,176);
		address+=176;

		dprintf_hanzi_string_1(a,0,0,16,0);
		dprintf_hanzi_string_1(b,0,16,16,0);
		dprintf_hanzi_string_1(c,0,32,16,0);
		dprintf_hanzi_string_1(d,0,48,16,0);
		if(FreqSelect == STOP){	
			TR0 = 0;
			while(1);
		}
